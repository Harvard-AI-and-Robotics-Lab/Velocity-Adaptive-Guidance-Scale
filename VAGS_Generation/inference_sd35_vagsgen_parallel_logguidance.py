...
# CSV logging helper
# ---------------------------------------------------------------------------
# A single in-process lock guarding writes to the log CSV. Each worker
# process has its own interpreter (so its own lock), and each worker
# writes to its own CSV (one per kappa, see run_experiment), so this
# lock only serialises concurrent calls within a single process.
_LOG_LOCK = threading.Lock()
_LOG_FIELDS = ["prompt_id", "step_index", "t", "sigma", "cos_sim",
               "lambda_i", "kappa", "base_lambda"]


def _append_log_rows(path: str, rows: List[Dict[str, Any]]) -> None:
    """Append rows to a CSV log, writing the header on first creation.

    Header creation and row appending are wrapped in a single critical
    section so two threads (or the pipeline + a callback) cannot race
    to create the file with no header.
    """
    if not rows:
        return
    with _LOG_LOCK:
...
class VAGGenSD3Pipeline(StableDiffusion3Pipeline):
    ...
    def __call__(
        ...
        vag_log_path: Optional[str] = None,
        vag_log_ids: Optional[List[str]] = None,
        **kwargs,
    ):
        ...
        # Optional per-step / per-sample log of the adaptive guidance scale.
        # Records are accumulated in memory and flushed to CSV after the
        # loop completes, so denoising itself touches no I/O.
        log_enabled = vag_log_path is not None
        log_records: List[Dict[str, Any]] = [] if log_enabled else None  # type: ignore
        if log_enabled:
            # Resolve the per-row prompt id. We always have `effective_batch`
            # rows per step. If the caller passed explicit ids we use them;
            # otherwise we fall back to the batch index, prefixed so it is
            # obviously synthetic.
            if vag_log_ids is not None:
                if len(vag_log_ids) != effective_batch:
                    raise ValueError(
                        f"vag_log_ids has {len(vag_log_ids)} entries but "
                        f"effective batch size is {effective_batch}."
                    )
                row_ids = [str(x) for x in vag_log_ids]
            else:
                row_ids = [f"sample_{b}" for b in range(effective_batch)]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if do_classifier_free_guidance:
                    ...
                    # Determine whether VAG-Gen modulation is in effect.
                    vag_active = vag_enabled and vag_kappa != 0.0

                    # We need (sigma_i, s_i) when VAG-Gen is active OR
                    # whenever logging is requested. Computing the cosine
                    # is cheap (one inner product per sample) so we just
                    # do it once and reuse.
                    need_stats = vag_active or log_enabled
                    ...
                    if vag_active:
                        ...
                        lambda_i_vec = guidance_scale * torch.exp(exponent)  # (B,)

                        # Reshape to broadcast against (B, C, H, W)
                        lambda_i_b = lambda_i_vec.to(v_uncond.dtype).view(-1, 1, 1, 1)
                        noise_pred = v_uncond + lambda_i_b * (v_cond - v_uncond)
                    else:
                        # Standard CFG. Per-sample lambda is just the constant.
                        noise_pred = v_uncond + guidance_scale * (v_cond - v_uncond)
                        if log_enabled:
                            lambda_i_vec = torch.full(
                                (effective_batch,),
                                float(guidance_scale),
                                device=v_uncond.device,
                            )

                    # ---- Per-step log accumulation ----
                    if log_enabled:
                        # Pull tensors back to CPU once per step (small: B floats).
                        sigma_val = float(sigma_i.detach().cpu().item())
                        t_val = float(t_norm.detach().cpu().item())
                        s_cpu = s_i.detach().cpu().tolist()
                        lam_cpu = lambda_i_vec.detach().cpu().tolist()
                        for b in range(effective_batch):
                            log_records.append({
                                ...
                                "t": t_val,
                                "sigma": sigma_val,
                                "cos_sim": float(s_cpu[b]),
                                "lambda_i": float(lam_cpu[b]),
                                "kappa": float(vag_kappa),
                                "base_lambda": float(guidance_scale),
                            })
        ...
        # 6b. Flush adaptive-guidance log to CSV (if enabled)
        if log_enabled and log_records:
            _append_log_rows(vag_log_path, log_records)
...
def parse_arguments():
    ...
    parser.add_argument("--log_adaptive_scale", action="store_true",
                        help=("If set, log the per-step / per-sample adaptive "
                              "guidance scale (and the inputs sigma_i, s_i) "
                              "to a CSV inside the output directory."))
    parser.add_argument("--adaptive_log_name", type=str, default="vagsgen_log.csv",
                        help="Filename of the adaptive-scale CSV log inside output_dir.")
...
def run_experiment(args):
    ...
    tmp_parent = os.path.dirname(args.output_dir)
    tmp_basename = os.path.basename(args.output_dir)
    vag_enabled = not args.disable_vag
    vag_log_csv = (
        os.path.join(tmp_parent, f'{tmp_basename}_guidance_analysis.csv')
        if args.log_adaptive_scale else None
    )
        ...
        if args.num_prompts_per_run == 1:
            ...
            for prompts, prompt_ids in zip(prompts_per_run, prompt_ids_per_run):
                ...
                result = pipe(
                    ...
                    vag_enabled=vag_enabled,
                    vag_log_path=vag_log_csv,
                    vag_log_ids=[str(x) for x in prompt_ids],
                )
    ...
    with open(os.path.join(args.output_dir, "runtime_inference.log"), "w") as f:
        ...
        f.write(f"Adaptive-scale log: {vag_log_csv}\n")
